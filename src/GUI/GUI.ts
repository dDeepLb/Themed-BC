...
export class GUISetting {
    ...
    load(mod: ModSDKModAPI<any>) {
        mod.hookFunction("PreferenceRun", HOOK_PRIORITY.OVERRIDE_BEHAVIOR, (args, next) => {
            ...
            if (PreferenceSubscreen === "" && GUISetting.instance !== null && GUISetting.instance.currentSubscreen === null) {
                if (window.BCResponsive_Loaded) {
                    DrawButton(1815, 710, 90, 90, "", "White", "Icons/Explore.png", Localization.GetText("setting_button_popup"));
                }
                else {
                    DrawButton(1815, 820, 90, 90, "", "White", "Icons/Explore.png", Localization.GetText("setting_button_popup"));
                }
            }
    });
        ...
        mod.hookFunction("PreferenceClick", HOOK_PRIORITY.OVERRIDE_BEHAVIOR, (args, next) => {
            ...
            if ( PreferenceSubscreen === "" && GUISetting.instance !== null && GUISetting.instance.currentSubscreen === null) {
                if( MouseIn(1815, 710, 90, 90) && window.BCResponsive_Loaded ) setSubscreen(new GUIMainMenu());
                if( MouseIn(1815, 820, 90, 90) && !window.BCResponsive_Loaded ) setSubscreen(new GUIMainMenu());
            } 
                return next(args);
        });
